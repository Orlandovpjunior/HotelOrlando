...
public class ReservaSingle extends Reservas {
	...
	public ReservaSingle(String idCliente,int numQuarto ,LocalDateTime dataInicio, LocalDateTime dataFim,Refeicao[] refeicoes,Quartos quarto, Usuario usuario, String tipo) {
	...
	public String toString() {
	    ...
	    if(this.isStatusPago() == false) {
	    	return String.format("[%d] Reserva de quarto em favor de:\n%s\nDetalhes da instalação:\n%s\nDetalhes da reserva:\nPeríodo: %s ate %s\nNo. de Hospedes: %d pessoa(s)\nRefeições incluídas:%s\nVALOR TOTAL DA RESERVA: R$%.2f x%d (diarias) => R$%s\nSITUACAO DO PAGAMENTO: PENDENTE.\n", 
			        this.getContador(),this.usuario.toString(), exibirQuarto(), dataInicioFormatada, dataFimFormatada, this.quarto.getQtdMaxPessoas(), Arrays.toString(refeicoes),valorDiaria() + (valorRefeicao(refeicoes) * this.quarto.getQtdMaxPessoas()), estadia(getDataInicio(), getDataFim()), this.formataValor(this.valorReserva()));
	    }else {
